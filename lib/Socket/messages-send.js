...
const retryWithBackoff = async (fn, maxRetries = 3, baseDelay = 1000) => {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            // Solo retry en errores específicos
            const shouldRetry = (
                error.data === 429 || // rate limit
            ...
            if (shouldRetry && attempt < maxRetries - 1) {
                ...
                console.warn(`Intento ${attempt + 1} falló, reintentando en ${delay}ms:`, error.message)
...
const cleanCorruptedSessions = async (authState, jids) => {
    try {
        if (!authState?.keys || !jids?.length) return
        
        const validJids = jids.filter(jid => jid && typeof jid === 'string')
        if (validJids.length === 0) return
        
        // Intentar limpiar sesiones problemáticas
        const sessionsToClean = {}
        for (const jid of validJids) {
            try {
                const signalId = authState.keys.signalRepository?.jidToSignalProtocolAddress?.(jid)
                if (signalId) {
                    sessionsToClean[signalId] = null
                }
            } catch (e) {
        ...
        if (Object.keys(sessionsToClean).length > 0) {
            await authState.keys.set({ 'session': sessionsToClean })
        }
    } catch (error) {
...
const makeMessagesSocket = (config) => {
    ...
    const { ev, authState, processingMutex, signalRepository, upsertMessage, query, fetchPrivacySettings, sendNode, groupQuery, groupMetadata, groupToggleEphemeral, newsletterWMexQuery, executeUSyncQuery } = suki   
    ...
    let mediaConn
    let lastQueryTime = 0
    const minQueryInterval = 1000 // 1 segundo mínimo entre queries

    // Query con rate limiting
    const rateLimitedQuery = async (queryData) => {
        const now = Date.now()
        const timeSinceLastQuery = now - lastQueryTime
        
        if (timeSinceLastQuery < minQueryInterval) {
            await new Promise(resolve => setTimeout(resolve, minQueryInterval - timeSinceLastQuery))
        }
        
        lastQueryTime = Date.now()
        return await retryWithBackoff(() => query(queryData))
    }
    ...
    const refreshMediaConn = async (forceGet = false) => {
        ...
        if (!media || forceGet || (new Date().getTime() - media.fetchDate.getTime()) > media.ttl * 1000) {
            mediaConn = (async () => {
                ...
                const result = await rateLimitedQuery({
                    tag: 'iq',
                    attrs: {
                        type: 'set',
                        xmlns: 'w:m',
    ...
    const sendReceipt = async (jid, participant, messageIds, type) => {
        ...
        await retryWithBackoff(() => sendNode(node))
    }
    ...
    const sendReceipts = async (keys, type) => {
        ...
        for (const { jid, participant, messageIds } of recps) {
            try {
                ...
                // Pequeña pausa entre receipts para evitar rate limiting
                await new Promise(resolve => setTimeout(resolve, 100))
            } catch (error) {
                logger.warn(`Error enviando receipt para ${jid}:`, error.message)
            }
        }
    }
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        jids = Array.from(new Set(jids.filter(jid => jid && typeof jid === 'string')))
        ...
        try {
            ...
            const result = await retryWithBackoff(() => executeUSyncQuery(query))
            ...
            logger.warn('Error en getUSyncDevices, usando dispositivos existentes:', error.message)
            // En caso de error, intentar usar lo que ya tenemos en cache
        }
    ...
    const assertSessions = async (jids, force) => {
        ...
        if (jidsRequiringFetch.length) {
            ...
            try {
                ...
                logger.warn('Error fetching sessions:', error.message)
                // Limpiar sesiones corruptas si hay error
                await cleanCorruptedSessions(authState, jidsRequiringFetch)
                throw error
            }
        }
    ...
    const createParticipantNodes = async (jids, message, extraAttrs) => {
        ...
        const nodes = await Promise.all(jids.map(async (jid) => {
            try {
                ...
                logger.warn(`Error encrypting for ${jid}:`, error.message)
                // Retornar null para jids que fallan, se filtrarán después
                return null
            }
        }))

        // Filtrar nodos null (aquellos que fallaron)
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, additionalNodes, AI = true }) => {
        ...
        return await retryWithBackoff(async () => {
            await authState.keys.transaction(async () => {
                ...
                if (isGroup || isStatus) {
                    const [groupData, senderKeyMap] = await Promise.all([
                        (async () => {
                            ...
                            if (groupData && Array.isArray(groupData?.participants)) {
                                ...
                                try {
                                    groupData = await retryWithBackoff(() => groupMetadata(jid))
                                } catch (error) {
                                    logger.warn(`Error obteniendo metadata del grupo ${jid}:`, error.message)
                                    // Usar datos básicos si no se puede obtener metadata
                    ...
                    if (!participant) {
                        ...
                        try {
                            ...
                            logger.warn('Error obteniendo dispositivos, continuando con dispositivos existentes:', error.message)
                        }
                    }
                    ...
                    if (senderKeyJids.length) {
                        ...
                        try {
                            ...
                            logger.warn('Error creando nodos de participantes:', error.message)
                            // Limpiar sesiones corruptas y continuar
                            await cleanCorruptedSessions(authState, senderKeyJids)
                        }
                    }
                    ...
                    if (!participant) {
                        ...
                        if (additionalAttributes?.['category'] !== 'peer') {
                            try {
                                ...
                                logger.warn('Error obteniendo dispositivos adicionales:', error.message)
                            }
                        }
                    }
                    ...
                    try {
                        ...
                        logger.warn('Error en assertSessions o createParticipantNodes:', error.message)
                        await cleanCorruptedSessions(authState, allJids)
                        throw error
                    }
                }
                ...
                await retryWithBackoff(() => sendNode(stanza))
            })

            return msgId
        }, 3, isGroup ? 2000 : 1000) // Mayor delay para grupos
    }
    ...
    const getEphemeralGroup = async (jid) => {
        ...
        return await retryWithBackoff(async () => {
            ...
            return WABinary_1.getBinaryNodeChild(metadata, 'ephemeral')?.attrs?.expiration || 0
        })
    }
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            }, WABinary_1.isJidGroup(jid) ? 3 : 2, WABinary_1.isJidGroup(jid) ? 2000 : 1000) // Más reintentos y delay para grupos
        }
    }
}
